...
export async function getPrice(marketId: number): Promise<{ buyPrice: string; sellPrice: string; buyAmount: string; sellAmount: string }> {
  const result = await fetch(`https://deepdex-api.deepdex.net/v1/blockchain/perp/order-books?market_id=${marketId}`).then(a => a.json());
  console.log({ result });
...
export async function getPairContract(pair: string): Promise<{ address: string; pairId: string | undefined }> {
  const result = await fetch(`https://deepdex-api.deepdex.net/perp/blockchain/perp/pairs`).then(a => a.json());
